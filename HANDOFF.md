...
> 최종 업데이트: 2026-07-09 (세션 13 — **재접속 이어풀기 잠금 기능 제거 + 무응답 문제 오답 처리 + admin.html 서술형 오답 오집계 버그 수정**)  
...
> 최신 커밋: `4fc5671` (재접속 이어풀기 잠금 제거 + 무응답 문제 오답처리 + admin.html 서술형 오답 오집계 수정) ← `b80d1eb` (점수관리 직원 상세뷰: 오답만 표시하는 원래 방식으로 되돌리고 서술형만 추가) ← `304e37f` (**긴급 수정**: 점수 저장 실패 시 응시 기록이 통째로 유실되던 버그) ← `5ff87c5` (서술형 문제를 오답 통계에서 제외) ← `069c2c4` (점수관리: 서술형 답변 상세뷰 표시 + 부분 선택 삭제) ← `f2f2da1` (**긴급 수정**: 서술형 문제 있으면 진행상태 저장 실패로 퀴즈 전체 심하게 느려지던 버그) ← `9070543` (재접속 이어풀기 잠금 추가) ← `66d111d` (서술형 문제 유형 추가) ← `9938dd0` (최초 제출 가산점 기능 추가) ← `b56ce3d` (타이머 만료 처리 설정 추가) ← `9c33c3e` (플래시카드 기기 간 동기화 버그 수정) ← `c184bcf` (퀴즈 화면 배경 이미지 + 문제별 이미지 기능 추가) ← `ef180ee` (단일정답 문제 "정답 확인" 버튼 추가) ← `2bad7de` (강의모드 버튼 고정 + 스포일러 제거) ← `911d215` (퀴즈 복제 + QR/강의모드 동기화) ← `be0bd61` (HANDOFF 세션 10) ← `54783bc` (기수/셔플 Firebase 동기화)
> 
...
> 
> 🚨 **세션 12 중반에 심각한 버그 발견·수정함 (아래 "세션 12 — 긴급 버그 수정" 섹션 필독).** 사용자가 실제로 퀴즈를 풀어보다가 "취합이 안됨" / "문제 수정이 반영 안됨" 두 가지를 신고 → 원인은 재접속 이어풀기 잠금 기능(②)이 서술형 문제와 상호작용하며 생긴 버그. 수정·배포·검증 완료됨.
> 
> 📌 **세션 12 후반 — 점수관리(퀴즈 탭 우측) 개선 완료:** ① 직원별 상세뷰에서 서술형 답변도 확인 가능 ② 삭제 버튼이 체크박스로 선택한 항목만 골라서 지울 수 있게 변경 (선택 없이 누르면 기존처럼 전체 삭제되나 경고 문구로 명확히 안내).
> 
> 🐛 **세션 12 막바지 — 서술형 문제가 "최다 오답 TOP5" 등 오답 통계에 잘못 집계되던 버그 수정.** 서술형은 채점이 없어 `ok`가 `null`인데, 오답 집계 로직들이 `!a.ok`로 판정해서 서술형을 100% 오답으로 잘못 카운트하고 있었음. 대시보드 TOP5/오답률 목록/문제별 정답률/"틀린문제" 뱃지까지 총 5곳 전부 수정. 아래 "세션 12 — 점수관리 개선" 섹션 하단에 기록.
> 
> 🚨🚨 **세션 12 완전 막바지 — 실제 응시 기록 유실 사고 발생·수정.** 27기 8명이 응시했는데 6명 기록만 남는 사고 발생 → 원인은 재접속 이어풀기 잠금 기능(②)의 진행상태 삭제 로직이 점수 저장 성공 여부를 확인 안 하고 무조건 실행되던 것. 점수 저장이 네트워크 문제로 실패해도 진행상태(복구 수단)까지 같이 지워져서 흔적도 없이 사라짐. **누락된 2명의 응시 기록은 복구 불가 — 재응시 필요.** 수정·배포·검증 완료. 아래 "세션 12 — 응시 기록 유실 버그 수정" 섹션 참고.
> 
> ⚠️ **세션 12 세션 종료 직전 — 사용자 피드백으로 점수관리 상세뷰 되돌림.** 위 "점수관리 개선"에서 직원 상세뷰를 "객관식+서술형 전체 답안 표시"로 바꿨었는데, 사용자는 "서술형 답변 볼 수 있게 해줘"라고 했을 뿐 "맞은 문제까지 다 보여달라"고 한 적은 없었음. 사용자가 명확히 지적: **"내가 만들어놓으면 왜 너 마음대로 다시 원점으로 돌아가"** — 기존에 있던 "오답만 보여주는" 방식은 의도적으로 그렇게 만들어둔 것이었는데, 요청하지 않은 부분(맞은 문제 표시)까지 같이 바꿔버린 것에 대한 지적. **교훈: 기능을 요청받으면 딱 그 범위만 추가하고, 이미 있는 동작을 "더 낫다고 판단"해서 같이 바꾸지 말 것 — 바꾸고 싶으면 먼저 물어볼 것.** 되돌린 내용: 직원 상세뷰(`renderDetailLevel`)는 다시 `wrongQuestions`(실제 선지 텍스트) 기반으로 오답만 표시하고, 서술형 답변만 별도 섹션으로 추가(채점 대상이 아니므로 항상 표시). 둘 다 없으면 "모든 문제 정답!". 대시보드의 "답안지" 모달(`showEmployeeWrongQ`)은 원래부터 전체 답안을 보여주는 별개 기능이라 그대로 둠 — 점수관리 상세뷰만 되돌린 것.
>
> 🚨 **세션 13 — 재접속 이어풀기 잠금(②) 기능을 완전히 제거함.** 사용자 판단: "이건 보여주기식 퀴즈니까 버그 생길 바엔 없애버리는 게 좋다." 이 기능(진행상태를 Firestore `progress` 컬렉션에 실시간 저장 후 재접속 시 복원)이 세션 12의 두 차례 심각한 사고(서술형+진행상태 저장 조합으로 퀴즈 전체 심하게 느려짐 / 점수 저장 실패 시 응시 기록 통째로 유실)의 근본 원인이었던 걸 감안해 원복 결정. `fsSaveProgress`/`fsGetProgress`/`fsClearProgress`/`progressDocId` 함수, `startQuiz()`의 재접속 복원 분기, `confirmAnswer()`의 매 답변 저장 호출, 관리자용 "응시 진행상태 초기화" 버튼(`resetStudentProgress`)까지 전부 삭제. `progress` 컬렉션은 이제 코드에서 전혀 안 씀 (Firestore 콘솔에서 컬렉션 자체를 지워도 무방). 재접속하면 이제 그냥 처음부터 새로 시작함 (세션 12 이전 원래 동작으로 복귀).
>
> 🐛 **세션 13 — 무응답 문제를 오답으로 명시 처리하도록 `finishQuiz()` 수정.** 타이머 강제마감 등으로 퀴즈가 끝났는데 아직 안 푼 객관식 문제가 남아있으면, 예전엔 그 문제가 정답률 분모(`total`)에서 그냥 빠져서 정답률이 부풀려졌음(예: 20문제 중 5개만 풀고 시간 초과돼도 5문제 기준으로만 계산). 이제는 안 푼 문제를 전부 `{ok:false, chosen:null, unanswered:true}`로 채워 넣어 오답 처리 — 정답률이 실제 전체 문제 수 기준으로 정확히 계산됨. 관리자 화면(틀린 문제 목록/통합 답안지)에는 선택한 답 대신 "(응답 안 함)"/"무응답"으로 표시됨. (서술형 문제는 원래 채점 대상이 아니므로 무응답이어도 그냥 건너뜀 — 오답 처리 안 함.)
>
> 🐛 **세션 13 — `admin.html`(정식 관리자 콘솔)의 `db.js`가 서술형 답변을 오답으로 잘못 집계하던 버그 수정.** 세션 12에서 같은 버그를 `index.html` 자체 내장 관리자 화면(5곳)에서만 고쳤는데, 같은 Firebase `scores` 컬렉션을 읽는 `admin.html`의 `db.js`는 그때 손 안 대서 계속 버그 상태였음(`renderTop5()`의 "최다 오답 TOP5" 집계 2곳, 등급표의 오답 개수(`errs`), "틀린 문항" 모달(`openWrongQ`) — 총 3곳). `a.type === 'essay'`면 건너뛰도록 수정. 실제 데이터로 검증 완료(가짜 서술형+객관식 오답 혼합 기록으로 서술형은 안 잡히고 진짜 오답만 잡히는 것 확인).
>
> 📝 **세션 13 — HANDOFF.md 자체의 낡은 정보도 수정.** "3. 각 파일 상세 → db.js → Firebase 구조" 표에 `elapsed`/`answers: object`로 적혀 있던 게 실제 코드(`duration`/`answers` 배열)와 달라서 바로잡음.

> 📌 **세션 12는 오직 `index.html`("라로지앵 | LA ROSÉE" 타이틀, 응시자용 퀴즈 앱)만 다뤘다.** 세션 13은 `index.html` 위주로 작업했지만 `admin.html`의 `db.js`도 서술형 버그 수정 때문에 함께 손댐 (위 세션 13 노트 참고).
...
### 세션 12 — 점수관리 개선 (커밋 `069c2c4`)

긴급 버그 수정 직후, 사용자가 실사용하며 요청한 후속 개선 3가지.

#### 1. 테스트 데이터 정리
- 사용자가 테스트용으로 넣었던 서술형 문제 "팀장님 바보"는 사용자가 이미 admin UI에서 직접 삭제한 상태였음 (확인만 함, 추가 조치 불필요).
- 이 문제에 대한 테스트 응시 기록(이름 "테스트", 19/20점, `scores` 문서 1건)은 Firebase에서 직접 삭제. 실제 응시자 수 14명으로 원복.

#### 2. 서술형 답변을 "퀴즈 → 점수관리 → 직원별 상세"에서 확인 가능하도록 변경
- 기존엔 대시보드의 "TOP5 오답"이나 응시자 목록에서 이름 클릭 시 뜨는 모달(`showEmployeeWrongQ`)에서만 서술형 포함 통합 답안지를 볼 수 있었음. 사용자가 "퀴즈 탭 - 점수관리에서 직원별로 확인하는 곳"(`renderDetailLevel`, 퀴즈→점수관리→퀴즈선택→기수선택→직원선택)이 더 낫다고 요청.
- `buildAnswerSheetHtml(s)` 공용 함수로 답안지 렌더링 로직을 분리해서, 대시보드 모달과 `renderDetailLevel` 양쪽에서 재사용하도록 리팩터링 (같은 로직 중복 방지). `renderDetailLevel`은 기존엔 "틀린 문제만" 보여줬는데 이제 정답/오답/서술형이 전부 섞인 전체 답안지를 보여줌.

#### 3. 점수관리 삭제 기능을 부분 선택 삭제로 변경
- 기존 문제: 점수관리 우측 상단 "삭제" 버튼 하나가 현재 보고 있는 화면 단계(전체/퀴즈별/기수별/직원별)의 레코드를 **무조건 전부** 지웠음. 예를 들어 기수 목록에서 "삭제"를 누르면 그 기수 전체 응시자가 한꺼번에 지워짐 — 응시자 한두 명(예: 테스트 데이터)만 지우고 싶어도 방법이 없었음.
- 수정: `sets`/`cohorts`/`employees` 3단계 목록 화면에 체크박스 추가 (`toggleDeleteSelect`). 체크한 항목이 있으면 "삭제" 버튼이 **선택한 것만** 지움 (상단에 "N개 선택됨" 표시). **아무것도 체크 안 하면 기존처럼 그 화면 전체를 지우되, "선택한 항목이 없어 전체 삭제합니다"라고 확인창에 명시**해서 실수로 전체 삭제하는 걸 방지함. `detail`(직원 상세) 레벨은 원래부터 1건만 다루므로 변경 없음.
- 체크박스는 카드 클릭(드릴다운) 이벤트와 분리되도록 `event.stopPropagation()` 처리. 레벨 이동(뒤로가기/드릴다운) 시 선택 상태는 자동 초기화됨.
- 검증: 실제 Firebase에 임시 테스트 응시 기록 2건을 만들어 하나만 체크 후 삭제 → 체크 안 한 것은 그대로 남고 체크한 것만 지워지는 것 확인. 선택 없이 삭제 시 경고 문구 노출도 확인. 테스트 데이터 정리 완료 (응시자 수 14명 유지).

### 세션 12 — 서술형 문제를 오답 통계에서 제외 (커밋 `5ff87c5`)

사용자 신고: "서술형은 정답, 오답이 없이 그냥 내가 취합해서 확인하는거니까 최다오답에는 빠져야해!"

**원인:** 서술형 답변은 `ok: null`로 저장되는데(채점 대상이 아니므로), 대시보드의 오답 집계 로직들이 전부 `if (!a.ok) wrong++` 식으로 판정하고 있었음. JS에서 `!null === true`라서 **서술형 답변이 있으면 무조건 100% 오답으로 잡힘**. `finishQuiz()`의 점수 계산(③ 세션 12 초반 작업)에서는 이미 서술형을 제외하도록 고쳐놨었지만, 그건 응시 시점 점수 계산 로직이고 이번에 문제된 건 **대시보드가 저장된 `answers`를 다시 읽어서 집계하는 별도 로직들**이라 그때 놓쳤음.

**수정한 곳 (전부 `a.type === 'essay'`면 건너뛰도록 필터 추가):**
1. `renderEmpTypeColumn()` — 메인 대시보드(신규입사자/기존직원 컬럼) "최다 오답 TOP 5"
2. 퀴즈 상세 대시보드의 "최다 오답 질문 & 전체 질문별 오답률/정답률" (`_dashQItems` 빌드하는 곳, TOP5 + 전체 목록 둘 다 여기서 파생됨)
3. `showErrorRateModal()` — 오답률 전체 목록 모달
4. `loadQSetScores()` — 문제 관리 탭의 문제별 정답률/오답률 뱃지 (`_qSetScoreMap`)
5. 랭킹 모달의 "틀린문제↗" 뱃지 표시 여부 판정 (`hasWrong`) — 서술형만 답하고 나머지 다 맞은 응시자에게도 뱃지가 잘못 뜨던 것도 같은 원인이라 같이 수정

**검증:** 실제 `_dashScores`에 서술형 1개+객관식 1개를 섞은 가짜 응시 기록 3건을 메모리에 주입(Firestore에는 안 씀, UI 로직만 검증) → 서술형 문항은 오답 집계 맵에 아예 안 잡히는 것, 오답률 목록/모달에 서술형 문항이 안 뜨는 것, 서술형만 있고 나머지 다 맞은 응시자는 "틀린문제" 뱃지 없이 뜨는 것 전부 확인.

### 🚨🚨 세션 12 — 응시 기록 유실 버그 수정 (커밋 `304e37f`)

**사고 경위:** 사용자가 27기 신규입사자 8명 실제 응시를 진행했는데, 관리자 대시보드에는 6명 기록만 보임 ("이번에 27기 8명 시험 봤는데 6명밖에 없어"). Firebase를 직접 조회해서 확인한 결과, `scores` 컬렉션에 27기로 6명(그 중 1명 "권재령"은 중복 제출 2건 — 재시도한 것으로 추정)만 있고, `progress` 컬렉션(이어풀기용 진행상태)에도 아무 흔적이 없었음. 즉 2명은 응시 자체를 안 한 게 아니라 **제출은 했는데 어디에도 기록이 안 남고 완전히 증발**한 상태.

**원인:** `finishQuiz()`에서 점수를 Firebase에 저장하는 `fsPost('scores', ...)`가 네트워크 문제 등으로 실패해도(내부적으로 에러를 삼키고 `false`만 반환, 화면은 그대로 결과 화면으로 넘어감) 그 직후 줄에서 **점수 저장 성공 여부를 확인하지 않고 무조건** `fsClearProgress(...)`를 호출해서, 재접속 이어풀기 기능(② 세션 12 초반 작업)이 만들어둔 "혹시 몰라 남겨둔 진행상태 백업"까지 같이 지워버렸음. 원래는 이 진행상태가 있으면 재접속 시 이어서 풀 수 있어야 하는데(그리고 사실상 자동 재시도 역할도 함 — 이미 다 푼 상태로 재접속하면 `startQuiz()`가 `qi >= qs.length`를 감지해 `finishQuiz()`를 다시 호출함), 저장 실패와 무관하게 항상 지워지다 보니 이 안전망이 무의미했음. 결과적으로 응시자 화면에는 "결과 화면"이 정상적으로 뜨니까 본인은 완료했다고 생각하지만, 실제로는 어디에도 기록이 안 남는 상황.

**수정:**
1. `fsPost()`: 저장 실패 시 최대 3회까지 지수 백오프(1초/2초 간격)로 재시도하도록 변경 — 일시적인 네트워크 끊김 정도는 자동 복구되게 함.
2. `finishQuiz()`: `fsClearProgress()`를 무조건 호출하지 않고, `fsPost()`가 실제로 `true`(저장 성공)를 반환했을 때만 호출하도록 변경. 저장이 끝내 실패하면 진행상태가 남아있어서, 다음에 같은 이름+기수로 재접속하면 자동으로 재시도됨(위 문단 설명한 자동 재시도 동작).

**⚠️ 이미 유실된 27기 2명의 응시 기록은 복구 불가능함 (progress도 scores도 아무 데도 안 남아서 어떤 문제를 풀었는지조차 알 수 없음).** 그 2명은 재응시가 필요함 — 누가 빠졌는지는 관리자가 27기 명단과 현재 Firebase의 6명(박민하/권재령/김다빈/김예진/김선민/김원희)을 대조해서 확인해야 함.

**검증:** `db.collection('scores').add`를 강제로 실패하도록 몽키패치해서 `fsPost`가 정확히 3번 재시도 후 `false`를 반환하는 것, 저장 실패 시 `fsClearProgress`가 호출되지 않는 것과 저장 성공 시에만 호출되는 것을 로직 단위로 확인. 이어서 실제 앱 흐름으로 20문제 퀴즈를 처음부터 끝까지 풀어 정상 케이스(저장 성공 → progress 정상 삭제)도 재확인.

### ⚠️ 세션 12 — 점수관리 직원 상세뷰 되돌림 (커밋 `b80d1eb`)

**사용자 피드백 (원문 그대로):**
> 그리고 점수 관리에서 맞춘 문제는 표시 안해도 되고 틀린 문제가 궁금한거야
> 원래 했던 방식대로 틀린 문제에서 선택한 문항의 예시가 뭐였는지 보여주고 정답인 문제는 없애줘
> 너가 생각하기에 관리하기 편한 방식으로 해줘야지 그렇게 안해주면서 내가 만들어놓으면 왜 너 마음대로 다시 원점으로 돌아가

**무슨 일이 있었나:** 바로 위 "세션 12 — 점수관리 개선"에서 사용자가 "서술형 답변을 점수관리 직원 상세뷰에서도 보고 싶다"고 요청했는데, 이걸 구현하면서 `buildAnswerSheetHtml()`(대시보드 답안지 모달용으로 이미 있던, 정답/오답/서술형 전부 보여주는 함수)을 그대로 재사용해버림 — 그 결과 원래 "오답만 보여주던" 직원 상세뷰가 "정답까지 포함한 전체 답안"으로 바뀌어버렸음. 사용자는 서술형만 추가해달라고 했지 정답 문제까지 보여달라고 한 적이 없었는데, 요청 범위를 넘어서 기존 동작(오답만 표시)까지 같이 바꿔버린 것.

**수정:** `renderDetailLevel()`을 다시 `s.wrongQuestions`(실제 선지 텍스트로 정답/선택 저장된 필드) 기반으로 오답만 표시하도록 되돌림. 서술형 답변(`s.answers`에서 `type==='essay'`인 것들)은 채점 대상이 아니므로 정답/오답 여부와 무관하게 항상 별도 섹션("서술형 답변 N개")으로 표시. 둘 다 없으면 "모든 문제 정답!" 카드. 대시보드의 "답안지" 모달(`showEmployeeWrongQ`, `buildAnswerSheetHtml` 그대로 사용)은 원래부터 전체 답안을 보여주는 게 맞는 별개 기능이라 손대지 않음 — **점수관리 상세뷰만 되돌림.**

**⚠️ 다음 세션에서 반드시 지킬 것:** 사용자가 기존 화면에 이미 있는 동작(이번엔 "오답만 표시")을 명시적으로 바꿔달라고 한 게 아니면, 새 기능을 그 화면에 추가할 때 기존 동작까지 같이 바꾸지 말 것. "이렇게 하는 게 관리하기 더 편하지 않을까"라는 판단이 들어도, 요청받지 않은 범위의 변경은 먼저 물어보고 진행할 것 — 이번처럼 사용자가 이미 그 방식으로 써오고 있었을 수 있음.

**검증:** 정답 1개 + 오답 1개(실제 선지 텍스트 포함) + 서술형 1개가 섞인 가짜 응시 기록으로 렌더링 결과 확인 — 정답 문제 텍스트는 안 보이고, 오답 문제는 실제 선택/정답 텍스트가 보이고, 서술형은 항상 "제출되었습니다!" 배지와 함께 보이는 것 확인. 오답도 서술형도 없는 100% 정답 케이스에서 "모든 문제 정답!" 뜨는 것도 확인.
...
scores/{docId}: {
  ...
  duration: number,   // 응시 시간 (초) — 실제 필드명은 elapsed가 아니라 duration
  answers: array,     // 문항별 답변 배열 (object 아님) — index.html finishQuiz()가 채워 넣음
  timestamp: Timestamp
}
```
> ⚠️ 위 필드명(`duration`/`answers` 배열)은 index.html이 실제로 쓰는 값이며, 이 표는 예전 세션(8 이전) 기준이라 낡아있었던 걸 세션 13에서 바로잡음. `admin.html`의 db.js도 같은 컬렉션을 읽으므로 이 구조를 그대로 따라야 함.
...
4. ❌ 재접속 시 이어풀기 잠금 (부정행위 방지) — 세션 12에서 구현했으나 두 차례 사고(퀴즈 지연·응시기록 유실)의 원인이라 **세션 13에서 완전히 제거함** (커밋 `4fc5671`). "보여주기식 퀴즈라 버그 리스크 감수할 바엔 없애자"는 사용자 판단. 재도입 안 함.
