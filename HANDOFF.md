...
> 🚨 **파일 수정은 반드시 Claude Code에서만. GitHub 웹 업로드 절대 금지. 이거 한 번 어기면 하루 토큰 날리고 5시간 기다린다.**
...
### 세션 7 — admin.html 전면 재작성 계획 수립
- **문제 진단 완료**: 부분 치환 방식이 구조적으로 불안정함을 확인
  - `admin.html` line 393: `alt="?쇰줈??>` → 닫는 `"` 없음 → HTML 파서가 `<input type="password">` 전체를 alt 속성값으로 삼킴 → 비밀번호 입력란 DOM에서 사라짐
  - `<title>` 포함 전체 한글 아직도 깨진 상태
- **해결 전략 결정**: 찾아바꾸기 대신 **admin.html 전면 재작성**
  - `efa5e8c_admin.html` (한글 완벽한 원본) + 현재 구조(외부 JS 참조, present-view 통합) 조합

### 세션 8 (현재) — admin.html 전면 재작성 + push 완료
- `efa5e8c_admin.html` + `efa5e8c_present.html` 기반으로 `admin.html` 전면 재작성
- 한글 완전 복원, 외부 JS 참조 유지, present-view 통합, 교육 진행 버튼 추가
- **커밋 `9a1216b` — push 완료**
...
| 파일 | 상태 |
|---|---|
| `admin.html` | ✅ 전면 재작성 완료 — 한글 복원, 커밋 `9a1216b` |
| `auth.js` | ✅ 정상 |
| `db.js` | ✅ 정상 |
| `quiz.js` | ✅ 정상 |
| `flashcard.js` | ✅ 정상 |
...
1. 사이트 접속 → 로그인 → 각 탭/버튼 동작 확인
2. 롤플레이 카테고리 내용 입력:
   - 현재 `핸드빌 기본` 카테고리만 내용 있음
   - 나머지 `빠른 고객`, `느린 고객`, `수분스틱`, `세럼`, `머드스틱`, `사워오일` 은 관리자 에디터에서 직접 입력 필요

---

## 4. 🚨 이것만 지키면 다시는 이 지옥 안 온다

> **이 규칙 하나 때문에 하루 토큰을 전부 날리고 5시간을 기다렸다.**  
> 절대 잊지 말 것.

### 규칙: 파일 수정은 반드시 Claude Code에서만

```
✅ Claude Code에서 수정 → git push         (이것만 해라)
...
**왜 이게 중요하냐:** GitHub 웹 인터페이스로 올리면 한글 UTF-8이 깨진다.  
깨진 한글이 HTML 속성 안에 들어가면 `"` 균형이 무너지고, 그 아래 DOM 전체가 사라진다.  
비밀번호 입력란이 사라진 것도 이것 때문이다 (`alt="?쇰줈??>` — 닫는 `"` 없음).  
이걸 찾아바꾸기로 고치려다 매 세션 토큰을 전부 쓰는 악순환이 반복됐다.

**한 번 규칙 어기면:** 또 하루 토큰 날리고 5시간 기다린다.

---

## 5. ⚠️ 절대 하지 말아야 할 것 (상세)
...
| 커밋 | 내용 |
|---|---|
| `9a1216b` | admin.html 전면 재작성 — 한글 완전 복원 (mojibake 수정) |
