...
> 최신 커밋: `2bad7de` (강의모드 버튼 고정 + 스포일러 제거) ← `911d215` (퀴즈 복제 + QR/강의모드 동기화) ← `be0bd61` (HANDOFF 세션 10) ← `54783bc` (기수/셔플 Firebase 동기화)
...
### 🔴 미해결 — "정답 누르면 바로 다음 문제로 자동 이동한다"는 사용자 보고, 재현 실패
사용자 보고: "퀴즈 풀어보니까 정답이든 오답이든 대답을 누르면 바로 다음으로 넘어가는데, 직원들이 직접 '다음' 버튼을 누를 수 있게 바꿔달라."

**직접 조사한 결과 (프리뷰에서 실제 `.click()`으로 재현 시도):**
- `renderQuestion()` → `selectAnswer(idx)` (단일 정답) / `confirmMultiAnswer()` (복수 정답) 흐름을 코드로 다 읽고, 실제 DOM에 렌더된 옵션 버튼을 `.click()`으로 눌러서 확인함.
- 단일 정답: 옵션 클릭 → 정답/오답 색칠 + `#q-next-wrap`("다음으로" 버튼) 표시 → **`qi`는 그대로**, "다음으로" 버튼을 직접 클릭해야만 `nextQuestion()`이 호출되어 `qi`가 증가함. 자동 이동 없음.
- 복수 정답: 옵션 여러 개 체크 → "확인" 버튼(`#q-confirm-wrap`) 클릭 → 정답/오답 색칠 + "다음으로" 버튼 표시 → 마찬가지로 직접 클릭해야 다음 문제로 이동. 자동 이동 없음.
- `.q-next-wrap`은 `position: fixed`라 스크롤 없이 항상 보이는 위치에 있음(퀴즈 화면은 애초에 이 버튼이 밀려나는 문제가 없었음 — 밀려나는 문제는 강의모드(`#s-lecture`)에만 있었음, 위 항목 1 참고).
- `setTimeout` 전수 검색해도 자동 진행시키는 타이머 로직 없음.

**결론: 현재 코드(커밋 `2bad7de` 기준)에는 자동 진행 로직이 없다.** 재현 못 한 이유 후보:
1. 사용자가 실제 배포 사이트(GitHub Pages)를 **강력 새로고침(Ctrl+Shift+R) 없이** 봐서 브라우저에 캐시된 예전 버전을 테스트했을 가능성 (이 프로젝트에서 반복적으로 발생하는 이슈, 섹션 6/7 참고).
2. 특정 기기(모바일)나 특정 퀴즈 세트에서만 재현되는 케이스를 놓쳤을 가능성.
3. 사용자가 실제로 본 것이 "정답/오답이 클릭 즉시 색칠되며 확정되는 것"(이건 의도된 동작, 되돌릴 수 없음)을 "다음으로 넘어간다"고 표현했을 가능성 — 실제로는 여전히 같은 문제 화면에 머물러 있고 "다음으로" 버튼만 누르면 되는 상태.

**다음 세션에서 할 일**: 사용자에게 강력 새로고침 후 재현되는지, 재현된다면 어느 화면/기기/퀴즈인지 구체적으로 물어보고 확인할 것. 재현되면 그때 가서 원인 조사.
