...
> 
> 📌 **세션 12 후반 — 점수관리(퀴즈 탭 우측) 개선 완료:** ① 직원별 상세뷰에서 서술형 답변도 확인 가능 ② 삭제 버튼이 체크박스로 선택한 항목만 골라서 지울 수 있게 변경 (선택 없이 누르면 기존처럼 전체 삭제되나 경고 문구로 명확히 안내). 아래 "세션 12 — 점수관리 개선" 섹션 참고.
